...
export default function Programmers() {
  ...
  const [progList, programSet] = useState(listOfAwesome);
  const [featuredId, featuredIdSetter] = useState(null);
  const getNameOfFeatured = () => {
    ...
    const progLocation = featuredId - 1;
    const progName = progList[progLocation].name;
    return progName;
  };
      ...
      <div className='programmers'>
        {
          ...
          progList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { featuredIdSetter(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
